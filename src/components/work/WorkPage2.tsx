...
import {blue, gold, grey, lime, red} from '@ant-design/colors';
...
export default function WorkPage2(props: Props) {
  ...
  // 휴일 조회
  const [, holdiRes, _getHoliday] = useAxios('get', '/v1/gowork/work/holiday');
  ...
  const _search = useCallback(() => {
    _select({year, month});
    _getHoliday({year, month});
  }, [_select, year, month, _getHoliday]);
  useEffect(() => {
    if (selectState === 'done') {
      if (isSelected) {
        _search();
                ...
                <View key={row} style={{flexDirection: 'row'}}>
                  {(() => {
                    ...
                    for (let col = 0; col < 7; col++) {
                      // 공휴일 여부
                      let holiday = null;
                      holdiRes?.forEach((holi: any) => {
                        if (
                          holi.date ===
                          `${year}${month}${String(forDay).padStart(2, '0')}`
                        ) {
                          holiday = holi;
                        }
                      });
